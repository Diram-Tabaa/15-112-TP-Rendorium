...
class Blender:
    ...
    def addObj(self, lst):
        self.fileOpener()
        objList = [["cube", "monkey"], ["cone", "cylinder"], ["uv_sphere"],
                   ["torus"]]
        bpy.ops.object.select_all(action='DESELECT')
        lst2 = []
        for ob in bpy.data.objects:
        ...
        for list in objList:
            if lst[0] in list:
                if objList[0] == list:
                    ...
                    exec(adder + "_" + lst[
                        0] + "_add(vertices = 64," + loco + ",scale=(0.5, 0.5, 0.5))")
                elif objList[2] == list:
                    exec(adder + "_" + lst[
                        0] + "_add(radius = 0.5,segments =64,ring_count = 32," + loco + ")")
                    bpy.ops.object.shade_smooth()

                else:
                    exec(adder + "_" + lst[
                        0] + "_add(major_segments=64, minor_segments=24," + loco + ")")
                    bpy.ops.object.shade_smooth()
    ...
    def materialUpdater(self, lst):
        ...
        bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
            "Principled BSDF"].inputs[1].default_value = valuesDict[
            "subsurface"]
        bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
            "Principled BSDF"].inputs[4].default_value = valuesDict["metallic"]
        bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
            "Principled BSDF"].inputs[15].default_value = valuesDict[
            "transmission"]
        bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
            "Principled BSDF"].inputs[0].default_value = col
        if valuesDict["subsurface"]:
            bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
                "Principled BSDF"].inputs[3].default_value = col
        else:
            bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
                "Principled BSDF"].inputs[3].default_value = (0, 0, 0, 0)
        if valuesDict["emission"]:
            bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
                "Principled BSDF"].inputs[17].default_value = col
        else:
            bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
                "Principled BSDF"].inputs[17].default_value = (0, 0, 0, 0)
        bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
            "Principled BSDF"].inputs[14].default_value = float(lst[3])
        bpy.data.materials["RendoriumMaterial" + lst[0]].node_tree.nodes[
            "Principled BSDF"].inputs[7].default_value = float(lst[2]) / 1000
    ...
    def renderAnimation(self, lst):
        ...
        bpy.ops.render.render(animation=True, use_viewport=False, layer='',
                              scene='')
    ...
    def cameraPan(self, lst):
        ...
        vec = mathutils.Vector((-z * float(lst[0]) / 1500,
                                z * float(lst[1]) / 1500, -float(lst[2]) / 40))
